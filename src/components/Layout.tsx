...
export function Layout() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <aside
        className={`
          fixed top-0 left-0 z-50 h-full w-64 sidebar-gradient border-r border-sidebar-border
      ...
      >
        <div className="flex items-center justify-between px-5 py-6 border-b border-sidebar-border">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl gradient-hero flex items-center justify-center shadow-hero">
              <GraduationCap size={18} className="text-sidebar-primary-foreground" />
            </div>
            <div>
              <h1 className="text-sm font-bold tracking-tight text-sidebar-foreground font-display">{APP_TITLE}</h1>
              <p className="text-xs text-sidebar-foreground/50">{APP_SUBTITLE}</p>
            </div>
          </div>
          <button
